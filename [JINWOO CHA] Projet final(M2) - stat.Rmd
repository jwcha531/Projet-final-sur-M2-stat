...
1-6. Est-ce que la t-valeur(statistics) chenge après avoir créé les données facultatives étant en loi normale ?  
...
```
Nous pouvons obtenir le résultat que la différence de deux groupes sont significatives car le p-valeur(2.2e-16) est inférieure à 0.05. S'il en est ainsi, nous ne pouvons pas adapter l'hypothèse nulle. La t-valeur(statistics) indique que la t-valeur(statistics) se situe de 0 (hypothèse nuelle) à droite 9.0611. La moyenne de l'écart maximal est faible : 797.9843 et élevé : 1240.4047. Il montre également que la moyenne de l'échantillon pour l'automatique, est 2503.143, et celle pour la manuelle, est 1483.949.   
...
Si nous réalisons l'expérience avec les données originales, nous pouvons observer que la distribution empirique est 0. Donc, nous pouvons rejeter l’hypothèse nulle car la t-valeur(statistics : 9.0611) n’existe pas graphiquement dans la loi noramle.  
...
Quand nous comparons deux données originales et facultatives, nous pouvons savoir comme ci-dessous,  
 ...
 2. Dans la distribution de la t-valeur(statistics), les données facultatives sont plus grandes que les données originales (24.25 > 9.0611)  
...
Si nous réalisons l'expérience avec les données originales, nous pouvons observer que la distribution empirique est 0. Donc, nous pouvons rejeter l’hypothèse nulle car nous ne pouvons pas pas vérifier la t-valeur(statistics) qui n’est pas graphiquement dans la distribution de t-valeur(statistics : -20.815).  
...
Comme les données originales, dans deux histogrammes sur les figures 12 et 13, nous pouvons savoir la différence entre deux groupes. En plus, la moyenne du gazole est plus grande que celle de l'essence car la ligne verticale de la moyenne de l'essence se situe plus gauche que celle du gazole. Seulement, cet histogramme est d'homoscédasticité de chaque groupe.  
...
Quand nous comparons deux données originales et facultatives, nous pouvons savoir comme ci-dessous,   
 ...
 2. Pour la t-valeur(statistics), les données originales sont plus grandes que les données facultatives (-20.815 > -48.613).  
...
summary(m)
```
Nous pouvons savoir que le poids de la voiture et la consommation de carburant sont très significatifs car la p-valeur(**2.2e-16**) est inférieure à 0.05.  
...
Et, ce résultat explique environ **42%** (R-sqaured : **0.4211**).   
...
predict(m, new_car_1)
```
Nous pouvons prédire un résultat que nous voulons. Si nous pouvons savoir la consommation de carburant, nous pouvons savoir le résultat de prédictition. Lorsque le poids de la voiture est 3222 kg, la consommation de carburant prédit 4.038262.  
...
Et, quand nous avons observé la différence entre deux groupes dans les données originales, bien que la distribution ne soit pas en loi normale, nous avons obtenu le même résultat par t-test. Et, nous avons observé le résultat après avoir créé les données étant en loi normale. Le résultat aussi, il est que deux données ne sont pas différente sur p-valuer. Cependant, il n'est que différent sur la t-valeur(statistics). C'est-à-dire, nous avons pu parler que plus le nombre d'obsevation augemente, plus la t-value est grande.  
